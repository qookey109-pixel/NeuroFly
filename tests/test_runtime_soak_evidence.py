...
def test_segmented_24h_soak_evidence_recomputes_and_passes() -> None:
    ...
    assert observed["first_run_number"] == 225
    assert observed["last_run_number"] == 458
    assert observed["run_count"] == 234
    assert observed["wall_clock_hours"] == 37.28
...
def test_all_frozen_runs_are_exactly_consecutive_successes() -> None:
    ...
    assert [item["run_number"] for item in runs] == list(range(225, 459))
