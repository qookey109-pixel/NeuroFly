...
CONFIG_SCHEMA = "neurofly-memory-boundary-state-localization-v0.1"
RECEIPT_SCHEMA = "neurofly-memory-boundary-state-localization-receipt-v0.1"
STATUS = "PREREGISTERED_EXPLORATORY_EXECUTION"
SCOPE = "real-malecns-reward-memory-boundary-state-localization"
...
def validate_config(config: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "question_frozen": config.get("question") == (
            "Which transient state class carries the extra-runtime benefit of exact "
            "three-tau prereward history into the identical terminal -20 through 0 "
            "memory-expression window?"
        ),
        "geometry_exact": (
...
def _visual_digest(value: Any) -> str | None:
    return None if value is None else _array_digest(value)


def _boundary_snapshot(inner: Any) -> dict[str, Any]:
    b = inner.brain
    return {
        "rate_kc": _array_digest(b.rate_kc),
        "rate_dan": _array_digest(b.rate_dan),
        "v": _array_digest(b.v),
        "g": _array_digest(b.g),
        "visual": _visual_digest(inner._last_visual_rgb),
        "memory": _plastic_edge_state(inner)["fraction_digest"],
    }


def _apply_boundary_intervention(inner: Any, condition: str) -> dict[str, Any]:
    b = inner.brain
    before = _boundary_snapshot(inner)

    if condition == "intact":
        pass
    elif condition == "clear_rule_traces":
        b.rate_kc[:] = b.initial["rate_kc"]
        b.rate_dan[:] = b.initial["rate_dan"]
    elif condition == "clear_membrane_conductance":
        b.v[:] = b.initial["v"]
        b.g[:] = b.initial["g"]
    elif condition == "clear_wrapper_visual_history":
        inner._last_visual_rgb = None
    else:
        raise ValueError(f"Unknown boundary condition: {condition}")

    after = _boundary_snapshot(inner)
    if before["memory"] != after["memory"]:
        raise RuntimeError("Boundary intervention mutated synaptic memory")

    allowed = {
        "intact": set(),
        "clear_rule_traces": {"rate_kc", "rate_dan"},
        "clear_membrane_conductance": {"v", "g"},
        "clear_wrapper_visual_history": {"visual"},
    }[condition]
    changed = {key for key in ("rate_kc", "rate_dan", "v", "g", "visual") if before[key] != after[key]}
    if not changed.issubset(allowed):
        raise RuntimeError(f"Boundary intervention changed non-target state: {changed - allowed}")
...
def _run_condition(
    *,
    ...
    none_boundary = _apply_boundary_intervention(none_inner, condition)
    true_boundary = _apply_boundary_intervention(true_inner, condition)

    terminal = _replay_rows(
        none_inner=none_inner,
        true_inner=true_inner,
        rows=terminal_rows,
    ...
    if none_memory_before != _plastic_edge_state(none_inner)["fraction_digest"]:
        raise RuntimeError("None branch memory changed during frozen recall")
    if true_memory_before != _plastic_edge_state(true_inner)["fraction_digest"]:
        raise RuntimeError("True branch memory changed during frozen recall")

    return {
        "condition": condition,
        "none_reset": none_reset,
        "true_reset": true_reset,
        "none_boundary": none_boundary,
    ...
    *,
    ...
    for condition in EXPECTED_CONDITIONS:
        ...
        if result["post_delay_fraction_digests"] != reference_pair:
            raise RuntimeError("Paired memory differs across boundary conditions")
...
def _aggregate(replicates: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for condition in EXPECTED_CONDITIONS:
        ...
        for lag in range(-TERMINAL_DECISIONS, 1):
            rows = [
                next(item for item in t["per_lag"] if item["lag"] == lag)
                for t in terminals
            ]
            per_lag.append({
                "lag": lag,
        ...
        out["conditions"][condition] = {
            ...
            "reward_cue_action_divergence_fraction": round(
                sum(t["reward_cue_action_divergence"] for t in terminals) / len(terminals), 12
            ),
            "mean_changed_kc_active_fraction": round(
...
def run_memory_boundary_state_localization(
    *,
    config_path: Path,
    base_checkpoint: Path,
    output_dir: Path,
    ...
    if not all(config_gates.values()):
        raise ValueError("Boundary-state localization config failed validation")
    ...
    evidence_gates = {
        ...
        "all_events_have_full_history": all(rep["event_index"] >= FULL_HISTORY_DECISIONS for rep in replicates),
        "all_memory_frozen": all(
            rep["conditions"][c]["memory_unchanged"]
            for rep in replicates for c in EXPECTED_CONDITIONS
        ),
        "all_condition_prefix_lags_exact": all(
            rep["conditions"][c]["prefix_lags"] == list(range(-60, -20))
            for rep in replicates for c in EXPECTED_CONDITIONS
        ),
        "all_terminal_lags_exact": all(
            rep["conditions"][c]["terminal"]["terminal_lags"] == list(range(-20, 1))
            for rep in replicates for c in EXPECTED_CONDITIONS
        ),
    ...
    body = {
        ...
        "status": (
            "EXPLORATORY_MEMORY_BOUNDARY_STATE_LOCALIZATION_COMPLETE"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--config", default="data/memory_boundary_state_localization_v01.json")
    parser.add_argument("--base-checkpoint", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--receipt", required=True)
    args = parser.parse_args(argv)
    result = run_memory_boundary_state_localization(
