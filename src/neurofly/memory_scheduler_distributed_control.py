...
CONFIG_SCHEMA = "neurofly-memory-scheduler-distributed-control-v0.1"
RECEIPT_SCHEMA = "neurofly-memory-scheduler-distributed-control-receipt-v0.1"
STATUS = "PREREGISTERED_EXPLORATORY_EXECUTION"
SCOPE = "real-malecns-reward-memory-scheduler-distributed-control"
EXPECTED_CONDITIONS = (
    "intact",
...
EXPECTED_REPLICATES = (("SD1",3501),("SD2",3511),("SD3",3527),("SD4",3533))
...
def run_study(*,config_path,base_checkpoint,output_dir,receipt_path):
    ...
    if not all(cg.values()): raise ValueError("Scheduler-distributed config failed validation")
    ...
    body={
        ...
        "status":"EXPLORATORY_MEMORY_SCHEDULER_DISTRIBUTED_CONTROL_COMPLETE" if valid else "INVALID_EXECUTION",
...
def main(argv=None):
    ...
    p.add_argument("--config",default="data/memory_scheduler_distributed_control_v01.json")
