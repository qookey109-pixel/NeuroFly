...
CONFIG_SCHEMA="neurofly-memory-cross-subsystem-localization-v0.1"
RECEIPT_SCHEMA="neurofly-memory-cross-subsystem-localization-receipt-v0.1"
STATUS="PREREGISTERED_EXPLORATORY_EXECUTION"
SCOPE="real-malecns-reward-memory-cross-subsystem-localization"
TARGET_REPLICATES=4
FULL_HISTORY_DECISIONS=60
POST_REWARD_DELAY_DECISIONS=5
CHANGED_TOLERANCE=1e-12
CANDIDATE_POOL=(5209,5227,5231,5233,5237,5261,5273,5279,5281,5297,5303,5309,5323,5333,5347,5351,5381,5387,5393,5399)
...
TARGET_FIELDS={
    "intact":(),
    "clear_vg_luminance":("v","g","luminance"),
    "clear_vg_delay":("v","g","refractory","queue","queue_count"),
    "clear_adaptation_luminance":("adaptation","luminance"),
    "clear_adaptation_delay":("adaptation","refractory","queue","queue_count"),
...
def validate_config(config:dict[str,Any])->dict[str,bool]:
    ...
    loc=config.get("localization_analysis") or {}
    scheduler=config.get("scheduler_control_policy") or {}
    ...
    return {
        ...
        "scheduler_retirement_exact":(
            scheduler.get("retired_from_this_round") is True
            and scheduler.get("no_scheduler_carrier_claim_opened") is True
        ),
        "localization_exact":(
            loc.get("reference")=="intact"
            and loc.get("broad_clear_reference")=="clear_broad_physical_transient"
            and loc.get("report_all_condition_minus_intact_deltas") is True
            and loc.get("report_broad_minus_condition_residuals") is True
            and loc.get("no_formal_significance_threshold") is True
            and loc.get("no_posthoc_pair_selection") is True
        ),
...
def _screen_candidates(*,base_checkpoint:Path,work_dir:Path):
    ...
    manifest={
        "schema":"neurofly-memory-cross-subsystem-selection-v0.1",
        ...
        "selected":[
            {
                "replicate_id":f"CS{i}","trajectory_seed":x["candidate_seed"],"event_index":x["event_index"],
...
def _apply_clear(inner:Any,condition:str)->dict[str,Any]:
    ...
    if before!=after:
        raise RuntimeError("Cross-subsystem clear mutated synaptic memory")
...
def _aggregate(reps):
    ...
    out["localization"]={}
    for condition in EXPECTED_CONDITIONS[1:-1]:
        cur=out["conditions"][condition]
        out["localization"][condition]={
            "condition_minus_intact":{m:round(cur[m]-intact[m],12) for m in METRICS},
            "broad_minus_condition":{m:round(broad[m]-cur[m],12) for m in METRICS},
        }
    return out
...
def run_study(*,config_path:Path,base_checkpoint:Path,output_dir:Path,receipt_path:Path):
    ...
    if not all(cg.values()):
        raise ValueError("Cross-subsystem localization config failed validation")
    ...
    body={
        ...
        "status":"EXPLORATORY_MEMORY_CROSS_SUBSYSTEM_LOCALIZATION_COMPLETE" if valid else "INVALID_EXECUTION",
...
def main(argv=None):
    ...
    p.add_argument("--config",default="data/memory_cross_subsystem_localization_v01.json")
